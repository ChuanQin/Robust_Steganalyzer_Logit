...
for i = 1:length(trained_ensemble)
    ...
    proj(find(isnan(proj))) = 0;
    votes = votes+sign(proj);
end
% probability
num_stego = (votes + length(trained_ensemble)) / 2;
results.probability = [1 - num_stego / length(trained_ensemble), num_stego /length(trained_ensemble)];
% logit = softmax(votes);
% results.logits = [1-logit, logit];
